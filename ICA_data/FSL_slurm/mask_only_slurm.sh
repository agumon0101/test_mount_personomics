...
ICA_MAPS=`${FSLDIR}/bin/remove_ext $1` ; shift # oh so this is done before already and shifted... so really output is the second thing here
...
new_dir_root="/ceph/chpc/shared/janine_bijsterbosch_group"
FSLurmDIR="${new_dir_root}/naranjorincon_scratch/NeuroTranslate/ty_script_ABCD_ICA/FSL_slurm"
OUTPUT=`${FSLDIR}/bin/remove_ext $1` ; shift # this output is an actual variable and its the extraction of the fist command of that scrupt so shift moves all positional arguments one step left 
...
echo ICA maps ${#ICA_MAPS}, ${ICA_MAPS} #/ceph/chpc/shared/janine_bijsterbosch_group/naranjorincon_scratch/NeuroTranslate/ty_script_ABCD_ICA/ABCD_ICA/ICAd15/melodic_IC
echo INPUTS ${#INPUTS} # 0 so empty
echo OUTPUTS ${#OUTPUT}, ${OUTPUT} # /ceph/chpc/shared/janine_bijsterbosch_group/naranjorincon_scratch/NeuroTranslate/ty_script_ABCD_ICA/ABCD_ICA/ICAd15/groupICA15.dr
echo LOG DIR ${#LOGDIR} ${LOGDIR} # /ceph/chpc/shared/janine_bijsterbosch_group/naranjorincon_scratch/NeuroTranslate/ty_script_ABCD_ICA/ABCD_ICA/ICAd15/groupICA15.dr/scripts+logs
echo DESIGN ${#DESIGN} ${DESIGN} #  -d /ceph/chpc/shared/janine_bijsterbosch_group/naranjorincon_scratch/NeuroTranslate/ty_script_ABCD_ICA/ABCD_ICA/ICAd15/design.mat -t /ceph/chpc/shared/janine_bijsterbosch_group/naranjorincon_scratch/NeuroTranslate/ty_script_ABCD_ICA/ABCD_ICA/ICAd15/design.con
echo FSL DIR: ${FSLDIR}
for i in $INPUTS ; do
  printf "inside for loop"
  eid=$( ${new_dir_root}/naranjorincon_scratch/NeuroTranslate/ty_script_ABCD_ICA/utils/fpath_to_eid.sh $i )
  if ! test -f "${OUTPUT}/mask_${eid}.nii.gz"
  then
	  echo "$FSLDIR/bin/fslmaths $i -Tstd -bin ${OUTPUT}/mask_${eid} -odt char" > "${LOGDIR}/drA_${eid}"
	  # version with parallel mask submission
  	  $FSLurmDIR/fsl_sub_slurm.sh -T 10 -N maskgen1 -l $LOGDIR -f "${LOGDIR}/drA_${eid}"
	  # version with sequential mask submission
