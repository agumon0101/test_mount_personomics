...
FSLurmDIR="/ceph/chpc/shared/janine_bijsterbosch_group/naranjorincon_scratch/NeuroTranslate/ty_script_ABCD_ICA/ICA_data/FSL_slurm" 
...
for i in $INPUTS ; do
  ...
  eid=$( /ceph/chpc/shared/janine_bijsterbosch_group/naranjorincon_scratch/NeuroTranslate/ty_script_ABCD_ICA/utils/fpath_to_eid.sh $i)  
#   echo "${FSLDIR}/bin/fsl_glm -i ${i} -d ${ICA_MAPS} -o ${OUTPUT}/dr_stage1_${eid}.txt --demean -m ${OUTPUT}/mask" > "${LOGDIR}/drC_${eid}"
#   $FSLurmDIR/fsl_sub_slurm.sh -T 30 -N dual_regression -l $LOGDIR -f "${LOGDIR}/drC_${eid}"
